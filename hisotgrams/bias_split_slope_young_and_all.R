# Slope histogram for Bias- and Bias+ trials in young and full samples.
# Input metric: slope from weibull_metric_results.csv.
# Output plot: output/plots/bias_split_slope_young_and_all.png
...
df_grouped_ws <- df_grouped %>%
  ...
  mutate(subject_mean = mean(slope, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(Sample, BiasLabel, Group) %>%
  mutate(group_grand_mean = mean(slope, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(slope_norm = slope - subject_mean + group_grand_mean)

df_summary <- df_grouped_ws %>%
  group_by(Sample, BiasLabel, Group, ExperimentName) %>%
  summarize(
    meanslope = mean(slope, na.rm = TRUE),
    sd_norm = sd(slope_norm, na.rm = TRUE),
    n = n_distinct(Subject),
    seslope = (sd_norm / sqrt(n)) * morey_correction,
    .groups = "drop"
  )
...
p <- ggplot(df_summary, aes(x = Group, y = meanslope)) +
  geom_col(
  ...
  geom_errorbar(
    aes(
      ymin = meanslope - seslope,
      ymax = meanslope + seslope,
...
stat_test_race_all <- df_grouped %>%
  group_by(Sample, BiasLabel, Group) %>%
  t_test(slope ~ ExperimentName, paired = TRUE) %>%
  add_significance("p") %>%
...
race_base_y <- df_summary %>%
  group_by(Sample, BiasLabel, Group) %>%
  summarize(y.position = max(meanslope + seslope, na.rm = TRUE) + 0.08,
            .groups = "drop")
...
stat_test_group <- df_grouped %>%
  ...
  t_test(slope ~ Group, paired = FALSE) %>%
  add_significance("p") %>%
  mutate(p.signif = ifelse(nchar(p.signif) > 3, "***", p.signif))
...
facet_y <- df_summary %>%
  ...
  summarize(
    facet_top = max(meanslope + seslope, na.rm = TRUE),
    facet_range = diff(range(meanslope + seslope, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
...
group_base_y <- df_summary %>%
  group_by(Sample, BiasLabel, ExperimentName) %>%
  summarize(group_base = max(meanslope + seslope, na.rm = TRUE), .groups = "drop")
...
ggsave(
  plot_path("bias_split_slope_young_and_all.png"),
  final_plot,
  width = 9,
  height = 8.5,
  dpi = 300,
