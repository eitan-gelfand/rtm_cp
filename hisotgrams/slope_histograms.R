# Subject-level slope summary plot from the Weibull fits.
# This figure summarizes the slope metric by Group, Race, and
...
# ──────────────────────────────────────────────────────────────
# 2) Aggregate to one slope value per subject x condition
# ──────────────────────────────────────────────────────────────
df_grouped <- df %>%
...
df_grouped_ws <- df_grouped %>%
  ...
  ungroup() %>%
  mutate(
    slope_norm = slope - subject_mean + group_grand_mean
  )
...
y_range <- diff(range(df_summary$meanslope, na.rm = TRUE))
...
# 4) Base histogram
# ──────────────────────────────────────────────────────────────
p_cp <- ggplot(df_summary, aes(x = Group, y = meanslope)) +
  geom_col(
    aes(fill = Regression),
...
stat_test_reg <- df_grouped %>%
  ...
  t_test(slope ~ Regression, paired = TRUE) %>%
...
reg_base_y <- df_summary %>%
  group_by(ExperimentName, Group) %>%
  summarize(
    reg_base = max(meanslope + seslope, na.rm = TRUE),
    reg_se_top = max(seslope, na.rm = TRUE),
    .groups = "drop"
  )
